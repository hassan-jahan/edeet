...
export default {
  mixins: [fieldMixin],
  props: ['block', 'parent', 'index', 'options', 'scope'],
  ...
  }, // parent can be an array or object with children same as action menus
  watch: {
    grandTotal(newVal) {
      this.value = { test: '123', total: newVal }
    },
  },
  created() {
    if ((typeof this.value === 'undefined' || this.value == '') && this.schema.slug) // testing to enable object value work without certain defination slug means its kind of form element (another way to do this is doing it only for that specific component that needs object value)
      ...
      <div class="container">
        <div class="header">
          ...
          <div class="d-flex justify-content-between">
            <p>
              Due Date:
              <b-form-input v-model="value" type="date" style="max-width: 200px;" />

              {{ value }}
              sdsds d
              <b-form-select
                id="currency-picker" v-model="invoiceCurrency" text-field="name"
                :options="currencies" style="max-width:100px"
              />
            </p>
          </div>
          ...
          <tr v-for="(item, index) in items" :key="index">
            ...
            <td data-label="Price/unite">
              <div class="cell-with-input">
                <b-form-input
                  v-model="item.price" :prefix="invoiceCurrency.symbol" type="number"
                  min="0"
                />
              </div>
            </td>
            <td data-label="Quantity">
              <b-form-input v-model="item.quantity" type="number" min="0" />
            </td>
            <td data-label="Total">
              {{ decimalDigits(item.price * item.quantity) }}
            </td>
            <td v-if="options.edit" class="text-right">
        ...
        <div class="d-flex flex-end justify-content-between align-content-start">
          <div>
            <b-button v-if="options.edit" class="mt-2" variant="success" size="sm" @click="addNewItem">
              Add item
            </b-button>
          </div>
            ...
            <tr>
              <td>
                <div class="cell-with-input">
                  Discount
                  <b-form-input
                    v-model="discountRate" append="%" class="text-right" type="number"
                    min="0" max="100"
                  />
                </div>
              </td>
            ...
            <tr>
              <td>
                <div class="cell-with-input">
                  Tax
                  <b-form-input
                    v-model="taxRate" append="%" class="text-right" type="number" min="0"
                    max="100"
                  />
                </div>
              </td>
