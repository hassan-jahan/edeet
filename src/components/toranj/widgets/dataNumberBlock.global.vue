...
export default {
  ...
  computed: {
    btnClasses() {
      ...
      return `dasht-btn btn btn-info d-inline-block ${(this.block.outline) ? 'dasht-btn-outline' : ''} ${this.block.rounded ? 'rounded-pill' : ''} ${this.block.size ? `btn-${this.block.size}` : ''} my-2 mx-1 `
    },
...
<template>
  ...
  <widget-wrapper
    ...
    style="pos22ition: rela22tive; display:inl2ine-block;"
